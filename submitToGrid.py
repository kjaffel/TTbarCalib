...
def submitProduction(tag,lfnDirBase,dataset,isData,cfg,workDir,lumiMask,submit=False):
    ...
    config_file.write('config.JobType.pyCfgParams = [\'defaults=Prompt18\', \'useTTbarFilter=True\', \'runFatJets=True\', \'miniAOD=True\', \'maxEvents=-1\', \'runOnData=%s\', \'groups=TTbar,JetInfo,PV,EventInfo,Josh,JetDeepFlavourPruned\']\n' % bool(isData))
    config_file.write('\n')
...
def main():
    ...
    githash = 'Moriond19_Run2018_DeepFlavourPruned_ttbarSysts'
    lfnDirBase='%s/%s/' % (opt.lfn,githash)
